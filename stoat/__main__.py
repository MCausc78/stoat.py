...
async def login(email: str, password: str, friendly_name: typing.Optional[str]):
    ...
    async with stoat.HTTPClient(state=state).attach() as http:
        ...
        if isinstance(resp, stoat.MFARequired):
            print('--- MFA required. ---')
            print('---------------------')
            print('|  Whats available  |')
            methods = resp.allowed_methods

            if stoat.MFAMethod.recovery in methods:
                print('|   Recovery code   |')
            if stoat.MFAMethod.totp in methods:
                print('|     TOTP code     |')
            print('---------------------')
        ...
        if isinstance(resp, stoat.AccountDisabled):
...
def _login(_parser: argparse.ArgumentParser, args: argparse.Namespace) -> None:
    if args.debug:
        stoat.utils.setup_logging(level=logging.DEBUG)
...
def parse_args() -> tuple[argparse.ArgumentParser, argparse.Namespace]:
    parser = argparse.ArgumentParser(prog='stoat', description='Tools for helping with stoat')
