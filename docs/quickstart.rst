...
3. We then use the :meth:`Client.on` decorator to register an event. This library has many events.
   ...
   the :class:`ReadyEvent` event is called when the bot has finished logging in and setting things
   up and the :class:`MessageCreateEvent` event is called when the bot has received a message.
4. Since the :class:`MessageCreateEvent` event triggers for *every* message received, we have to make
   sure that we ignore messages from ourselves. We do this by comparing :attr:`User.relationship` value
   to :attr:`RelationshipStatus.user`.
