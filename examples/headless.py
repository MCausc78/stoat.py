...
async def main() -> None:
    ...
    state = stoat.State()
    async with stoat.HTTPClient(token=token, state=state).attach() as http:
        # Sending a message in Stoat > Lounge
        message = await http.send_message(channel_id, 'Hello world')
        print('Message was sent in Stoat > Lounge, with ID', message.id)
        ...
        elif isinstance(channel, stoat.SavedMessagesChannel):
            print('This channel is your personal "Saved Notes" channel')
        elif isinstance(channel, stoat.DMChannel):
